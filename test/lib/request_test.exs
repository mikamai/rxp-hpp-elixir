defmodule RequestTest do
  ...
  test "build_hash with hpp_select_stored_card is overrides payer_ref" do
    request = %Request{valid_hpp_request() | payer_ref: "newpayer1"}
    request2 = %Request{
      valid_hpp_request() |
      payer_ref: "newpayer1", hpp_select_stored_card: "HPP_SELECT_STORED_CARD"
    }
    request_hash = Request.build_hash(request, "mysecret").sha1hash
    request2_hash = Request.build_hash(request2, "mysecret").sha1hash
  ...
  test "build_hash without hpp_select_stored_card does not override payer_reference" do
    request = %Request{valid_hpp_request() | payer_ref: "newpayer1"}
    request2 = %Request{valid_hpp_request() | hpp_select_stored_card: ""}
    request_hash = Request.build_hash(request, "mysecret").sha1hash
    request2_hash = Request.build_hash(request2, "mysecret").sha1hash
